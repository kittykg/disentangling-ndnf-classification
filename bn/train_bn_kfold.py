...
from bn.train_bn import (
    cosine_with_exponential_decay_and_warmup_scheduler,
    loss_calculation,
)
...
def train_fold(
    ...
    device: torch.device,
    use_wandb: bool,
) -> tuple[BooleanNetworkClassifier, dict[str, float]]:

    # Model
    model = construct_model(training_cfg, bn_dataset.data.shape[2])
    model.to(device)
    ...
    for epoch in range(training_cfg["epochs"]):
        ...
        if use_wandb:
            ...
            if isinstance(model, BooleanNetworkNeuralDNF):
                wandb_log_dict[f"fold_{fold_id}/train/delta"] = old_delta
            ...
            if log_lr_in_wandb:
                # Log learning rate
                wandb_log_dict[f"fold_{fold_id}/train/lr"] = (
