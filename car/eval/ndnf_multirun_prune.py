...
def multiround_prune(
    ...
    prune_eval_function = lambda: parse_eval_return_meters_with_logging(
        eval_meters=car_classifier_eval(model, device, train_loader),
...
def single_model_prune(
    ...
    _eval_with_log_wrapper("Car NDNF (after training, test)", test_loader)
    log.info("------------------------------------------")
    ...
    prune_eval_log = _eval_with_log_wrapper(
        "Car NDNF pruned (test)", test_loader
    )
    log.info("============================================================")
