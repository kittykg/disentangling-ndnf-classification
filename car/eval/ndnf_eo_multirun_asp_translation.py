...
from car.models import CarNeuralDNF, CAR_NUM_CLASSES
...
def asp_eval(
    ...
    for d in test_data:
        ...
        if disjunction_syntax == "PRED":
            asp_base += [
                f"#show {disjunction_name}_{i}/0."
                for i in range(CAR_NUM_CLASSES)
            ]
        else:
    ...
    prediction_tensor = torch.zeros(
        len(predictions), CAR_NUM_CLASSES, dtype=torch.float32
    )
...
def multirun_asp_translate(cfg: DictConfig):
    ...
    with open("asp_eval_results.json", "w") as f:
