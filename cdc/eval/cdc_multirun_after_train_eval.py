"""
This script evaluates a CDCClassifier model. The input models are strictly
...
def multirun_after_train_eval(cfg: DictConfig) -> None:
    ...
    # for NDNF invented predicate discretisation
    single_ipd_eval_results: list[dict[str, float]] = []
    for s in eval_cfg["multirun_seeds"]:
        ...
        model_name = "CDC-NDNF" if is_ndnf else "CDC-MLP"

        eval_log = parse_eval_return_meters_with_logging(
            cdc_classifier_eval(
                model, device, test_loader, discretise_invented_predicates=False
            ),
            model_name=model_name,
            do_logging=True,
            metric_prefix="nipd/" if is_ndnf else "",
        )
        single_eval_results.append(eval_log)
        json_dump_log = eval_log

        if is_ndnf:
            eval_log_ipd = parse_eval_return_meters_with_logging(
                cdc_classifier_eval(
                    model,
                    device,
                    test_loader,
                    discretise_invented_predicates=True,
                ),
                model_name=model_name + " - IPD",
                do_logging=True,
                metric_prefix="ipd/",
            )
            single_ipd_eval_results.append(eval_log_ipd)
            json_dump_log = {**json_dump_log, **eval_log_ipd}

        with open(model_dir / f"after_train_eval_result.json", "w") as f:
            json.dump(json_dump_log, f, indent=4)
    ...
    if is_ndnf:
        relevant_keys = list(single_ipd_eval_results[0].keys())
        for k in relevant_keys:
            for sk, sv in synthesize(
                np.array([d[k] for d in single_ipd_eval_results])
            ).items():
