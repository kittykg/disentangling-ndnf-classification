"""
This script translates CDC NeuralDNF model with weights only in the set {±6, 0}
to its logically-equivalent ASP program. It also translates the invented
predicates into a readable format. The input model is strictly after pruning
stage and a discretisation step (either thresholding/disentanglement) in the
post-training processing pipeline. The ASP program and the predicate translation
are stored and evaluated, with the relevant information stored in a json. The
evaluation metrics include accuracy, precision, recall, F1 score and MCC.
"""
...
from cdc.models import CDCNeuralDNF, CDC_NUM_BINARY_FEATURES
...
def load_discretised_model(
    ...
    model = CDCNeuralDNF(
        invented_predicate_per_input=eval_cfg["model_architecture"][
...
def translate_invented_predicate(
    ...
    number_of_invented_predicates = (
        model.ndnf.conjunctions.weights.shape[1] - CDC_NUM_BINARY_FEATURES
    )
    for conj_w in model.ndnf.conjunctions.weights.data:
        for i, w in enumerate(conj_w):
            if i >= number_of_invented_predicates:
                break
            if w != 0:
...
def single_model_translate(
    ...
    model = load_discretised_model(model_dir, cfg["eval"])
...
def multirun_asp_translate(cfg: DictConfig):
    ...
    scaler_mean, scaler_var = get_scaler_mean_var_from_path(cfg["dataset"])
    ...
    for s in eval_cfg["multirun_seeds"]:
        ...
        ret_dicts.append(
            single_model_translate(cfg, scaler, model_dir, test_data_with_label)
        )
        log.info("============================================================")
