"""
This script prunes the CDCNDNF model. The input models are strictly after
...
def multiround_prune(
    ...
    prune_eval_function = lambda: parse_eval_return_meters_with_logging(
        eval_meters=cdc_classifier_eval(model, device, train_loader),
...
def single_model_prune(
    ...
    _eval_with_log_wrapper("CDC NDNF (after training, test)", test_loader)
    log.info("------------------------------------------")
    ...
    prune_eval_log = _eval_with_log_wrapper(
        "CDC NDNF pruned (test)", test_loader
    )
    log.info("============================================================")
