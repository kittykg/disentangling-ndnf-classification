"""
This script disentangle pruned CDC NeuralDNF model. The input models are
strictly after pruning stage in the post-training processing pipeline. The
disentangled CDC NeuralDNF models are stored and evaluated, with the relevant
information stored in a json. The evaluation metrics include accuracy,
...
def threshold_intermediate_model_disjunctive_layer(
    intermediate_model: CDCNeuralDNF,
    device: torch.device,
...
def single_model_disentangle(
    ...
    def _eval_with_log_wrapper(
        ...
        eval_meters = cdc_classifier_eval(model, device, loader)
        return parse_eval_return_meters_with_logging(eval_meters, model_name)

    # Stage 1: Evaluate the pruned model
    _eval_with_log_wrapper(model, "Pruned CDC NDNF (test)", test_loader)
    log.info("------------------------------------------")

    # Stage 2: Disentangle the pruned model
    def disentangle(model: CDCNeuralDNF) -> dict[str, Any]:
        ...
        intermediate_pre_threshold_log = _eval_with_log_wrapper(
            model, "Intermediate disentangled CDC NDNF model (val)"
        )
        log.info("------------------------------------------")
        ...
        intermediate_thresholed_log = _eval_with_log_wrapper(
            model,
            f"Thresholded disentangled CDC NDNF (t={best_t_val}) (val)",
        )
        log.info("------------------------------------------")
        ...
        intermediate_pruned_log = _eval_with_log_wrapper(
            model, "Pruned disentangled CDC NDNF (test)", test_loader
        )
        log.info("------------------------------------------")
        ...
        condensed_model_log = _eval_with_log_wrapper(
            model, "Condensed disentangled CDC NDNF (test)", test_loader
        )
    ...
    if model_path.exists() and disentangle_result_json.exists():
        ...
        disentangled_model: CDCNeuralDNF = ret["condensed_model"]
