"""
This script compares the performance of CDC NDNF model after train vs after
...
def single_model_comparison(model_dir: Path) -> dict[str, Any]:
    ...
    disentangle_v2_result_json = model_dir / AFTER_DISENTANGLE_V2_JSON
    ...
    for k in ["accuracy", "f1"]:
        performance_loss_dict[f"train_threshold_{k}"] = (
            train_result[f"ipd/{k}"]
            - threshold_result["threshold_final_log"][k]
        )
        performance_loss_dict[f"train_disentangle_{k}"] = (
            train_result[f"ipd/{k}"]
            - disentangle_result["condensed_model_log"][k]
        )
        performance_loss_dict[f"train_disentangle_v2_{k}"] = (
            train_result[f"ipd/{k}"]
            - disentangle_v2_result["condensed_model_log"][k]
        )
