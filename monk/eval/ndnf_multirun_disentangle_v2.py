"""
This script disentangle pruned Monk NeuralDNF model. The input models are
strictly after pruning stage in the post-training processing pipeline. The
disentangled Monk NeuralDNF models are stored and evaluated, with the
...
from monk.eval.ndnf_multirun_prune import multiround_prune
...
def threshold_intermediate_model_disjunctive_layer(
    intermediate_model: MonkNeuralDNF,
    device: torch.device,
    ...
    for v in t_vals:
        ...
        threshold_eval_dict = monk_classifier_eval(
...
def single_model_disentangle(
    ...
    def _eval_with_log_wrapper(
        ...
        eval_meters = monk_classifier_eval(model, device, loader)
        return parse_eval_return_meters_with_logging(eval_meters, model_name)

    # Stage 1: Evaluate the pruned model
    _eval_with_log_wrapper(model, "Pruned Monk NDNF (test)", test_loader)
    log.info("------------------------------------------")

    # Stage 2: Disentangle the pruned model
    def disentangle(model: MonkNeuralDNF) -> dict[str, Any]:
        ...
        intermediate_pre_threshold_log = _eval_with_log_wrapper(
            model, "Intermediate disentangled Monk NDNF model (val)"
        )
        log.info("------------------------------------------")
        ...
        intermediate_thresholed_log = _eval_with_log_wrapper(
            model,
            f"Thresholded disentangled Monk NDNF (t={best_t_val}) (val)",
        )
        log.info("------------------------------------------")
        ...
        intermediate_pruned_log = _eval_with_log_wrapper(
            model, "Pruned disentangled Monk NDNF (test)", test_loader
        )
        log.info("------------------------------------------")
        ...
        condensed_model_log = _eval_with_log_wrapper(
            model, "Condensed disentangled Monk NDNF (test)", test_loader
        )
    ...
    if model_path.exists() and disentangle_result_json.exists():
        ...
        disentangled_model: MonkNeuralDNF = ret["condensed_model"]
