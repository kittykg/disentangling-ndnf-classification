"""
This script evaluates a MonkClassifier model. The input models are strictly
...
from monk.models import MonkMLP, construct_model
...
def multirun_after_train_eval(cfg: DictConfig) -> None:
    ...
    for s in eval_cfg["multirun_seeds"]:
        ...
        eval_log = parse_eval_return_meters_with_logging(
            monk_classifier_eval(model, device, test_loader),
            f"Monk-{'MLP' if isinstance(model, MonkMLP) else 'NDNF'}",
            do_logging=True,
        )
