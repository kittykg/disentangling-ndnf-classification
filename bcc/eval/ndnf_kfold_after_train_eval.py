...
def single_model_eval(
    model: BCCNeuralDNF, device: torch.device, val_loader: DataLoader
) -> tuple[dict[str, float], dict[str, float]]:
    def _eval_with_log_wrapper(
        model_name: str, discretise_invented_predicates: bool
    ) -> dict[str, float]:
        eval_meters = bcc_classifier_eval(
            model, device, val_loader, discretise_invented_predicates
        )
        return parse_eval_return_meters_with_logging(eval_meters, model_name)

    no_ipd_eval_log = _eval_with_log_wrapper(
        "BCC-NDNF (no discretising invented predicate)", False
    )
    ipd_eval_log = _eval_with_log_wrapper(
        "BCC-NDNF (no discretising invented predicate)", True
    )
    log.info("============================================================")

    return no_ipd_eval_log, ipd_eval_log
...
def post_train_prune(cfg: DictConfig) -> None:
    ...
    no_ipd_ret_dicts: list[dict[str, float]] = []
    ipd_ret_dicts: list[dict[str, float]] = []
    for fold_id, (_, test_index) in enumerate(skf.split(X, y)):
        ...
        no_ipd_eval_log, ipd_eval_log = single_model_eval(
            model, device, val_loader
        )
        no_ipd_ret_dicts.append(no_ipd_eval_log)
        ipd_ret_dicts.append(ipd_eval_log)
    ...
    relevant_keys = list(no_ipd_ret_dicts[0].keys())
    return_dict = {}
    for k in relevant_keys:
        for sk, sv in synthesize(
            np.array([d[k] for d in no_ipd_ret_dicts])
        ).items():
            return_dict[f"no_ipd_{k}/{sk}"] = sv
        for sk, sv in synthesize(
            np.array([d[k] for d in ipd_ret_dicts])
        ).items():
            return_dict[f"ipd_{k}/{sk}"] = sv
