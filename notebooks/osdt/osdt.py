...
class Tree:
    ...
    def __init__(
        self, cache_tree, ndata, lamb, splitleaf=None, prior_metric="curiosity"
    ):
        ...
        if leaves[0].num_captured == ndata:
            ...
            if num_cap_rm < ndata:
                self.metric = sum(
                    self.entropy[i] for i in range(l) if splitleaf[i] == 0
                ) / (ndata - sum(leaf.num_captured for leaf in removed_leaves))
            else:
                self.metric = (
                    sum(self.entropy[i] for i in range(l) if splitleaf[i] == 0)
                    / 0.01
                )
            ...
            if num_cap_rm < ndata:
                self.metric = sum(
                    self.giniindex[i] for i in range(l) if splitleaf[i] == 0
                ) / (ndata - sum(leaf.num_captured for leaf in removed_leaves))
            else:
                self.metric = (
                    sum(
...
def log(
    ...
    the_old_tree = str(
        0
    )  # str(sorted([leaf.rules for leaf in tree_old.cache_tree.leaves]))
    ...
    the_new_tree_depth = str(
        0
    )  # str(max([len(leaf.rules) for leaf in tree_new.leaves]))

    the_queue = str(
        0
    )  # str([[ leaf.rules for leaf in thetree.leaves]  for _,thetree in queue])
    ...
    line = ";".join(
        [
            ...
            the_Rc,
            the_old_tree,
            the_old_tree_splitleaf,
            the_old_tree_objective,
            the_old_tree_lbound,
            the_new_tree,
            the_new_tree_splitleaf,
            the_new_tree_objective,
            the_new_tree_lbound,
            the_new_tree_length,
            the_new_tree_depth,
            the_queue,
...
def generate_new_splitleaf(
    unchanged_leaves, removed_leaves, new_leaves, lamb, R_c, incre_support
):
    """
    ...
    splitleaf1 = [0] * n_unchanged_leaves + [
        1
    ] * n_new_leaves  # all new leaves labeled as to be split
    ...
    for i in range(n_removed_leaves):
        ...
        a_l = (
            removed_leaves[i].loss
            - new_leaves[idx1].loss
            - new_leaves[idx2].loss
        )
...
class OSDT:
    ...
    def fit(self, x, y):
        ...
        tree0 = Tree(
            ...
            ndata=ndata,
            splitleaf=[1],
            prior_metric=self.prior_metric,
        )
        ...
        best_is_cart = (
            False  # a flag for whether or not the best is the initial CART
        )
        if self.init_cart:  # if warm start
            # CART
            clf = sklearn.tree.DecisionTreeClassifier(
                max_depth=(
                    None if self.MAXDEPTH == float("Inf") else self.MAXDEPTH
                ),
        ...
        if self.readTree:
            ...
            sorted_new_tree_rules = tuple(
                sorted(leaf.rules for leaf in d_c.leaves)
            )
        ...
        while (
            queue and COUNT < self.niter and time.time() - tic < self.timelimit
        ):
            # tree = queue.pop(0)
            ...
            if self.lenbound and new_tree_length >= min(
                old_tree_length + math.floor((R_c - tree.lb) / self.lamb),
                max_nleaves,
            ):
            ...
            lb = tree.lb
            b0 = (
                sum([leaf.B0 for leaf in removed_leaves])
                if self.equiv_points
                else 0
            )
            ...
            rules_for_leaf = [
                ...
                - set(map(abs, l.rules))
                - set(
                    [i + 1 for i in range(nrule) if l.is_feature_dead[i] == 1]
                )
            ...
            for leaf_rules in product(*rules_for_leaf):
                    ...
                    for new_rule in (-rule, rule):
                        ...
                        if new_rules not in leaf_cache:
                            ...
                            tag_rule = (
                                x_mpz[rule_index]
                                if new_rule_label == 1
                                else ~(x_mpz[rule_index]) | mpz(pow(2, ndata))
                            )
                            # print("x_mpz",x_mpz)
                        ...
                        if (
                            self.accu_support == True
                            and (
                                new_leaf.num_captured
                                - new_leaf.num_captured_incorrect
                            )
                            / ndata
                            <= self.lamb
                        ):
                ...
                for new_leaf_split in new_leaf_splits:
                    ...
                    if (
                        len(new_leaf_split) + sum(new_leaf_split)
                        > self.MAX_NLEAVES
                    ):
                        continue
