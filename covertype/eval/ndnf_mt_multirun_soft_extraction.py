"""
This script thresholds pruned CoverTypeNDNFMT model. The input models are
...
class BaseChainedNeuralDNF(CoverTypeBaseNeuralDNF):
    ...
    def __init__(
        ...
        super().__init__(
            invented_predicate_per_input=predicate_inventor.invented_predicate_per_input,
            num_conjunctions=sub_ndnf.conjunctions.out_features,
            predicate_inventor_tau=1.0,
        )
        self.predicate_inventor = predicate_inventor
...
def disentangle_conjunctive_layer(
    ...
    chained_ndnf = MutexTanhChainedNeuralDNF(
        predicate_inventor=model.predicate_inventor,
...
def single_model_soft_extract(
    ...
    def process_conjunctive_layer_disentangle() -> dict[str, Any]:
        ...
        if (model_dir / "temp_chained.pth").exists():
            ...
            predicate_inventor = NeuralDNFPredicateInventor(
                num_features=model.predicate_inventor.num_features,
                invented_predicate_per_input=model.predicate_inventor.invented_predicate_per_input,
                tau=model.predicate_inventor.tau,
            )
            chained_model = MutexTanhChainedNeuralDNF(
                predicate_inventor=predicate_inventor,
                sub_ndnf=NeuralDNF(
        ...
        #         device=device,
        #     )
        #     test = chained_model(x, discretise_invented_predicate=True)
        #     print(test)
