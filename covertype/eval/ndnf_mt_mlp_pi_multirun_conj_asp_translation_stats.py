"""
This script translates Covertype NeuralDNF-MT (MLP Predicate Inventor)'s
conjunctive layer with weights only in the set {+/-6, 0} to its
logically-equivalent ASP program. The input model is strictly after pruning
stage and soft-extraction (either thresholding/disentanglement) in the
post-training processing pipeline. This script only translates and does not
evaluate.
"""
...
from covertype.eval.eval_common import DEFAULT_GEN_SEED
from covertype.eval.ndnf_mt_mlp_pi_multirun_soft_extraction import (
...
from covertype.models import CoverTypeMLPPINeuralDNF

log = logging.getLogger()

MT_MLP_PI_CONJ_ASP_TRANSLATION_THRESHOLDED_JSON_BASE_NAME = (
    "mt_mlp_pi_conj_translation_thresholded"
)
MT_MLP_PI_CONJ_ASP_TRANSLATION_DISENTANGLED_JSON_BASE_NAME = (
    "mt_mlp_pi_conj_translation_disentangled"
)
...
def load_discretised_model(
    model_dir: Path,
    eval_cfg: DictConfig,
) -> CoverTypeMLPPINeuralDNF | BaseChainedNeuralDNF:
    ...
    model_architecture_cfg = eval_cfg["model_architecture"]
    predicate_inventor_dims = model_architecture_cfg[
        "predicate_inventor_mlp_dims"
    ]
    n_conjunctions = model_architecture_cfg["n_conjunctions"]
    c2b = eval_cfg.get("convert_categorical_to_binary_encoding", False)
    ...
    if discretisation_method == "threshold":
        ...
        # The soft-extracted thresholded model is of type CoverTypeMLPPINeuralDNF
        model = CoverTypeMLPPINeuralDNF(
            predicate_inventor_dims=predicate_inventor_dims,
            num_conjunctions=n_conjunctions,
            c2b=c2b,
        )
    ...
    # Uses Covertype's MutexTanhChainedNeuralDNF
...
def single_model_translate(
    ...
    model = load_discretised_model(model_dir, eval_cfg)
    ...
    if discretisation_method == "threshold":
        translation_json_path = (
            model_dir
            / f"{MT_MLP_PI_CONJ_ASP_TRANSLATION_THRESHOLDED_JSON_BASE_NAME}.json"
        )
        ...
        translation_json_path = (
            model_dir
            / f"{MT_MLP_PI_CONJ_ASP_TRANSLATION_DISENTANGLED_JSON_BASE_NAME}{cd_version_str}.json"
        )
    ...
    rules_as_dict = extract_asp_rules(
        (
            ...
            if isinstance(
                model, CoverTypeMLPPINeuralDNF
            )  # Thresholded model is CoverTypeMLPPINeuralDNF
            else model.sub_ndnf.state_dict()  # Disentangled model is MutexTanhChainedNeuralDNF
        ),
        ...
        json.dump(
            {
                ...
                for k, v in rules_as_dict.items()
            },
            f,
            indent=4,
        )
...
def multirun_asp_translate(cfg: DictConfig):
    ...
    for s in eval_cfg["multirun_seeds"]:
        ...
        # Pass cfg for model architecture details
        ret_dicts.append(single_model_translate(cfg, model_dir))
        log.info("============================================================")
    ...
    with open(
        "mt_mlp_pi_soft_extraction_asp_translation_stats_results.json", "w"
    ) as f:
    ...
    try:
        ...
        if use_discord_webhook:
            ...
            post_to_discord_webhook(
                ...
                experiment_name=f"{cfg['eval']['experiment_name']} Multirun MLP-PI Soft-Extraction ASP Translation + Stats",
