"""
This script thresholds pruned plain NDNF from the CoverType MLP PINDNF-EO model.
The input models are strictly after pruning stage in the post-training
processing pipeline. The thresholed NDNF models are stored and evaluated. The
    ...
    def threshold(do_logging: bool = False) -> dict[str, Any]:
        ...
        for v in t_vals:
            ...
            if DEFAULT_COMPUTE_ERROR_DICT:
                error_meter = threshold_eval_dict["error_meter"]
                assert isinstance(error_meter, ErrorMeter)
                overall_error_rate = error_meter.get_average()[
                    "overall_error_rate"
                ]
